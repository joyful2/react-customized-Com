...
class ColumnItem extends Component {
  render() {
    const { render = () => "占位符", title } = this.props.item;
    return <td>{title}</td>;
  }
}
  ...
  render() {
    ...
    return (
      <thead>
        <tr>
          {columns.map((item) => (
            <ColumnItem item={item} key={item.key} />
          ))}
        </tr>
      </thead>
    );
...
class DataSourceItem extends Component {
  render() {
    ...
    const tds = columns.map((item) => (
      <td>
        {item.render
          ? item.render(dataItem[item.dataIndex], dataItem, index)
...
class DataSource extends Component {
  render() {
    ...
    let trs = dataSource.map((item, index) => (
      <DataSourceItem dataItem={item} index={index} columns={columns} />
    ));
